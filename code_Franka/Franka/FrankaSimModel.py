...
class FrankaSimModel():
    def __init__(self, name='Franka'):
        """
        :param: name: string
            name of objective
        """
        ...
        # joint handle
        self.joint_handle = [-1, -1, -1, -1, -1, -1, -1]
    ...
    def initializeSimModel(self, client_ID):
        ...
        for i in range(7):
            return_code, self.joint_handle[i] = vrep_sim.simxGetObjectHandle(client_ID, 'Franka_joint' + str(i+1), vrep_sim.simx_opmode_blocking)
    ...
    def getJointAngle(self, joint_name):
        ...
        :param: joint_name: str
            the joint name: can be Franka_joint1 ~ Franka_joint6
        """
        for i in range(7):
            if joint_name == 'Franka_joint' + str(i+1):
    ...
    def setJointAngle(self, joint_name, q):
        """
        :param: joint_name: str
            the joint name: can be Franka_joint1 ~ Franka_joint7
        :param: q: float array of size 7 x 1
            the desired joint angle for all joints
        """

        for i in range(7):
            if joint_name == 'Franka_joint' + str(i+1):
                vrep_sim.simxSetJointTargetPosition(self.client_ID, self.joint_handle[i], q, vrep_sim.simx_opmode_oneshot)
                return
...
if __name__ == "__main__":
    ...
    for t in np.linspace(0, t_max, t_max*100):
        q = Franka_sim_model.getAllJointAngles()
        print(q)

        desired_q = [0, 0, np.sin(t), 0.5*np.sin(t), 0.8*np.sin(t), 0, 0]
        for i in range(7):
            Franka_sim_model.setJointAngle('Franka_joint'+ str(i+1), desired_q[i])
